...
var rqifsub = "";
var rqmobile="";
define(["../weChat/merchandiseShare",
        "../weChat/wechatEvent"],function(require) {
	
	var weChatShare = require("http://localhost:8088/js/src/model/weChat/merchandiseShare");
	var share = require("http://localhost:8088/js/src/model/weChat/wechatEvent");
	function shareCallBack(){
		var image = common.getNewBaseUrl()+"/images/517share.png";
		var shareObject = {};
		shareObject.title="天天1G大转盘";
		shareObject.link="http://www.zhg3.com/P/repeat/index.action?toUrl=http://www.zhg3.com:38079/fuliclient/testIndex/testIndex.do";
...
//	weChatShare.weChatShare.ready(shareCallBack,true);
	  rqmobile=common.getHttpParam("rqmobile");
	  alert(rqmobile);
...
//	getPhoneKey();
    ...
    $("#notify-ul").notifyAnimate({
        speed:2000,
        item:"li",
        url:"../js/"+SCRIPT_PATH+"/model/bigwheel/notify-list.txt"
    });
...
function getUserChance(){
	...
	if(!isTime){
		common.G(common.getNewBaseUrl()+ "/bigwheel/index.html");
		return;
	}
	if(rqmobile != "null"){
		...
		$.ajax({
			url : common.getNginxUrl()+ "fuliclient/testIndex/getUserChance.do"+common.GetRequestStr(),
	        type: "post",
	        dataType: "json",
...
//	        				goMyProduct();
	        	...
	        	if(parseFloat(data.totalChance) <= 0){
	                ...
	                var url = "http://www.zhg3.com/P/repeat/index.action?toUrl="+common.getBaseUrl()+"testIndex/fuliBusiness.do?actityId="+actityId+"&mobileno="+phoneTxt;
...
function getPhoneKey(){
		$.ajax({
			url : common.getBaseUrl()+ "testIndex/getPhoneKey.do",
	        type: "post",
	        dataType: "json",
	        beforeSend:function(data){ 
	        }, 
...
function clashSpecialProduct(e){
	if(rqifsub == "1"){
		var id = e ; 
		$.ajax({
	        url: common.getBaseUrl() + "testIndex/clashSpecialProduct.do?id="+id,
	        type: "post",
	        dataType: "json",
	        beforeSend:function(data){ 
	            $.loading();
...
function drawLucky(){
	...
	if(phoneTxt == ""){
		...
		if(parseFloat(chance) <= 0){
        	...
        	$("#btn-go").unbind( "click" ) ;
        	$.ajax({
    			url : common.getBaseUrl()+ "testIndex/drawLucky.do",
    	        type: "post",
    	        dataType: "json",
    	        beforeSend:function(data){ 
	    ...
	    animate:function(){
	        ...
	        if(dialAnimate.minTime >= 2.5){
	            if(Math.round(dialAnimate.animateOpt.animateDelay) > 100){
	                //console.log(dialAnimate.minTime);
	            ...
	            if(Math.round(dialAnimate.animateOpt.animateDelay) <= 500){
	                //console.log(dialAnimate.animateOpt.animateDelay);
	                dialAnimate.animateOpt.animateDelay = Math.floor(dialAnimate.animateOpt.animateDelay + (oldTime * 0.2));
	            }
	        }
...
//adv_btn1
function adv_btn2(){
	window.location =common.getBaseUrl() + "testIndex/advbtn1.do";
//	alert(123);
//	$.ajax({
//        url: common.getBaseUrl() + "testIndex/advbtn1.do",
//        type: "get",
//        asyne: true,
//        dataType: "json",
//        beforeSend:function(data){ 
...
function adv_btn1(){
	window.location =common.getBaseUrl() + "testIndex/advbtn2.do";
//	$.ajax({
//        url: common.getBaseUrl() + "testIndex/advbtn2.do",
//        type: "get",
//        asyne: true,
//        dataType: "json",
//        beforeSend:function(data){ 
...
function adv_btn3(){
	window.location =common.getBaseUrl() + "testIndex/advbtn3.do";
//	$.ajax({
//        url: common.getBaseUrl() + "testIndex/advbtn3.do",
//        type: "get",
//        asyne: true,
//        dataType: "json",
//        beforeSend:function(data){ 
...
//跳转营销方案
function toyingxiao(){
	window.location = common.getBaseUrl() + "testIndex/toyingxiao.do";
//	$.ajax({
//        url: common.getBaseUrl() + "testIndex/toyingxiao.do",
//        type: "post",
//        asyne: true,
//        dataType: "json",
//        beforeSend:function(data){ 
...
function checkTotalChance(e){
	...
	if(parseFloat(chance) <= 0){
        ...
        $("#dial-inner").addClass("share-qrcode-show");
        $("#dial-main").addClass("dial-animate");
        var url = "http://www.zhg3.com/P/repeat/index.action?toUrl="+common.getBaseUrl()+"testIndex/fuliBusiness.do?actityId="+actityId+"&mobileno="+phone;
        var shortUrl = getShortUrl (url);
        $("#qrcode-wrap").empty().qrcode(shortUrl);
    }else{
...
function showProduct(e){
	...
	if(resourceId == "11990"){
        ...
        if(rqifsub == "1"){
        	testObj["查看奖品"] = function(){
            		window.location =common.getBaseUrl() + "testIndex/userAccountPage.do";
            };
